...
# Function to display game app selection menu
show_game_menu() {
    # Define game array with: name, description, install script, selected (true/false)
    declare -a games=(
        "Lutris|Open gaming platform for Linux|./bin/games/install-lutris.sh|false"
    ...
    for item in "${games[@]}"; do
        ...
        game_map["$display_key"]="$name|$desc|$script"
    done

    # Show game App selection menu
    gum style --foreground 212 --bold "Game Installation"
    echo ""
    gum style "Select Game Apps to install (space to toggle, Enter to confirm):"
    echo ""
    ...
    if [[ -z "${selected:-}" ]]; then
        gum style --foreground 244 "No Game Apps selected."
        exit 0
    fi

    # Parse selected items and run corresponding scripts
    echo ""
    gum style --foreground 212 --bold "Installing selected Game Apps.."
    echo ""
    ...
    for selected_item in "${selected_array[@]}"; do
        # Get the game info from the map
        if [[ -n "${game_map[$selected_item]:-}" ]]; then
            IFS='|' read -r name desc script <<< "${game_map[$selected_item]}"
    ...
    echo ""
    gum style --foreground 40 --bold "Game App installation complete!"
}
